...
Once an indicator is created, it is assigned a hexadecimal UUID in the format `8-4-4-4-12` (a 128-bit number) as its name, which is used for deletion in our script. This explains why it is possible to create an indicator without a name, as the UUID serves as its name and unique identifier.
